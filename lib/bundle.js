    ...
    for (let i = 0; i < 20; i++) {
      for (let j = 0; j < 20; j++) {
        if (allBubbles[i][j].display === 1) {
          ...
          // console.log(allBubbles);
          if (Math.abs(allBubbles[i][j].y-initialY) < 40 && (Math.abs(allBubbles[i][j].x-initialX) < 40)) {
            // PUT IN BREAK
            dx = 0;
            dy = 0;
            shoot = false;
...
// Main game loop
  ...
  function mainGame() {
    ...
    // image.onload = function() {
      ctx.drawImage(image, 190, 250, 90, 90, 255 , 600, 40, 40);
      // ctx.save();
      // ctx.beginPath();
      // ctx.arc(255, 600, 20, 0, Math.PI*2, false);
      // ctx.clip();
      // ctx.closePath();
